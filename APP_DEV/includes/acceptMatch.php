...
if (isset($_POST['sender']) && isset($_POST['receiver']) && isset($_POST['timestamp'])) {
    ...
    $stmtDogID->fetch();
    $stmtDogID->close();
    $updateQuery = "UPDATE tblnotifications SET type = 'Success' WHERE sender = ? AND receiver = ? AND timestamp = ?";
    $stmtUpdate = $conn->prepare($updateQuery);
    $stmtUpdate->bind_param("sss", $receiverUsername, $sendername, $timestamp);
    $stmtUpdate->execute();
    $stmtUpdate->close();
    $insertQuery = "INSERT INTO tblmatch (uploader, interestedUser, dogID, timestamp) VALUES (?, ?, ?, NOW())";
